#!/usr/bin/env bash
# docs/api/openapi.yaml から Redoc の HTML を docs/api/redoc.html に作る
# proto を変えたら just generate の後に流し、生成物と一緒にコミットする
# 前提: Node (npx) が使えること
...
# Redoc 本体の CSS にある、ブラウザが無視する記述を消す (エディタの警告が生成物に出続けるため)
#   - 接頭辞のない font-smoothing と tap-highlight-color は存在しないプロパティ
#   - ドットが 2 つ続くセレクタは不正で、規則ごと適用されない
# CLI は CSS を 1 行 1 規則に圧縮して出すため、その形に合わせて消す
sed -i.bak -E \
