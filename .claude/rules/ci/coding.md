...
- **CI から just を呼ばない。** 実行のたびに just の導入が要るため、コマンドを直接書く。justfile と同じ内容を 2 か所に持つことになるので、片方を変更したらもう片方も修正する
