...
コメント・規約・ドキュメント・Issue・PR・コミットメッセージは技術文書の書き言葉で書く。話し言葉寄りの和語や、古い・くだけた言い回しを避ける。

- 操作や状態の変化は、漢語のサ変動詞で書く (作る → 作成する、終わる → 完了する、消す → 削除する、変える → 変更する、足す → 追加する)
- IT の用語は、業界で定着したカタカナ語を使う (版 → バージョン、窓 → ウィンドウ)
- 迷ったら、公式ドキュメントの日本語版 (Google Cloud・Terraform) での表記に合わせる
