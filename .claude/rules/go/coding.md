...
機能で切ったうえで、その中を層に切る。数を決めない (守るのは依存の向きであり層の数ではない)。
...
- **テストは対象のファイルと 1 対 1 で対応させる** (`create_park.go` ↔ `create_park_test.go`)。共有するフェイクとヘルパは `<pkg>_test.go` に置く
...
| 層             | 検証するもの                                   | 例                                    |
| -------------- | ---------------------------------------------- | ------------------------------------- |
| `handler`      | 形式と型。protovalidate がインターセプタで行う | 必須フィールド、文字列長、日付の形式  |
